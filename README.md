...
- 📊 **行情数据**: AKShare全A股实时行情 + 历史K线(eastmoney失败自动回退sina)
- 📈 **策略引擎**: 内置+用户自定义策略，信号管道，回测框架
- 🧠 **AI决策**: DeepSeek情绪分析、形态识别、AI选股
- 🛡️ **风控管理**: T+1锁定、仓位限制、多级熔断、交易时段强制
- 🖥️ **交易与看盘**: 手动交易、持仓管理、涨停分析(首板/连板)、个股K线详情
- ⚡ **实时推送**: WebSocket 报价/信号/风控告警(toast)
- 🔐 **JWT认证**: 登录鉴权
